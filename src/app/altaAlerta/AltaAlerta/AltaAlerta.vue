<template>
  <div :class="$style.altaAlerta">
    <vue-grid>
      <vue-breadcrumb :items="breadCrumbItems"></vue-breadcrumb>

      <vue-grid-row>
        <vue-grid-item fill>
          <vue-headline level="1">AltaAlerta</vue-headline>
        </vue-grid-item>
      </vue-grid-row>
    </vue-grid>
  </div>
...
export default {
  ...
  computed: {
    breadCrumbItems() {
      return [
        ...
        { label: this.$t('common.AltaAlerta' /* AltaAlerta */), href: '/alta-alerta' },
      ];
    },
  },
  /*beforeCreate() {
    registerModule('altaAlerta', AltaAlertaModule);
  },
  prefetch: (options: IPreLoad) => {
     ...
     *
     * return options.store.dispatch('fetchAltaAlerta', '1');
     *
