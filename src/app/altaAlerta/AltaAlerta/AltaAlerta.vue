<template>
  <div>
    ...
    <v-row>
      ...
      <v-col :cols="screenType">
        <v-card outlined v-show="dispositivoSuelo">
        ...
        <v-card outlined v-show="dispositivoGps">
          <v-select
            v-model="gps"
            :items="opcionesGps"
            :rules="[v => !!v || 'opcion es requerida']"
...
AL
    this.map2.on('pm:remove', (e) => {
      if (e.layer instanceof L.Rectangle && JSON.stringify(e.layer.toGeoJSON()) !== this.fincaElement) {
        this.map2.pm.addControls({
          drawRectangle: true,
        });
