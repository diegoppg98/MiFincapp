        ...
        <v-col :cols="posMap">
          ...
          <v-card>
            ...
            <div class="text-center pt-2">
              ...
              ></v-text-field>
              <v-text-field v-model="numberItems" label="Número de Medidas" type="number"></v-text-field>
            </div>
          </v-card>
          <v-card-actions>
            <v-dialog
              ...
              <v-card>
                ...
                <v-form class="my-1" dark ref="form" v-model="valid" :lazy-validation="lazy">
                  ...
                  <v-text-field
                    ...
                    disabled
                  ></v-text-field>
                  <v-text-field class="my-1 mx-3" v-model="tipo" label="Tipo" disabled></v-text-field>
                  <v-text-field
                    class="my-1 mx-3"
            ...
            <v-dialog v-model="dialog" max-width="290" style="z-index: 999">
              <v-card>
                ...
                <v-card-text>Esta seguro de que desea eliminar el dispositivo, este procesos será irreversible.</v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="#2e7d32" text @click="dialog = false">Cancelar</v-btn>
                  <v-btn color="#7d2f2e" text @click="eliminarDispositivo">Eliminar</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </v-card-actions>
...
import { Medida } from 'app/Clases/Medida';
...
export default {
  ...
  data(): any {
    return {
      ...
      numberItems: 30,
      search: '',
      headers: [{ text: 'Tiempos de llegada', value: 'tiempo' }],
      nameRules: [
  ...
  watch: {
        ...
        if (this.getPivot !== null) {
          ...
          L.geoJSON(L.geoJSON(JSON.parse(localizacion[0])).toGeoJSON(), {
            onEachFeature: function (feature, layer) {
              var p;
              if (
                t.localizacion[0].getLatLng().lat === feature.geometry.coordinates[0][1] &&
                t.localizacion[0].getLatLng().lng === feature.geometry.coordinates[0][0]
              )
                var p = L.latLng(feature.geometry.coordinates[1][1], feature.geometry.coordinates[1][0]);
              else var p = L.latLng(feature.geometry.coordinates[0][1], feature.geometry.coordinates[0][0]);
    ...
    numberItems(val) {
      ...
      FactoryAPI.getFactoryAPI('Firebase')
        .getMedida()
        .listMeasurements(this.id, parseInt(val))
        .then((result) => {
          if (result !== null) {
            result.forEach(function (childResult:Medida) {
  ...
  methods: {
        ...
        this.localizacionDispositivo.forEach(function (element, index) {
          ...
          if (t.tipo == 'GPS')
            t.calculoPosicion(
              ...
              t.positions,
              t.coordenadaPivot,
              t.coordenadasActualesDispositivo,
              t.tipo,
              true,
            );
          else {
    ...
    clicked(value) {
      if (this.tipo === 'GPS') {
        ...
        if (this.tipoPivot === 'lineal') {
          ...
          if (
            Math.floor(this.coordenadaPivot.getLatLngs()[0].lat * Math.pow(10, 4)) / Math.pow(10, 4) ===
              Math.floor(this.coordenadasActualesDispositivo[1] * Math.pow(10, 4)) / Math.pow(10, 4) &&
            Math.floor(this.coordenadaPivot.getLatLngs()[0].lng * Math.pow(10, 4)) / Math.pow(10, 4) ===
              Math.floor(this.coordenadasActualesDispositivo[0] * Math.pow(10, 4)) / Math.pow(10, 4)
          ) {
    ...
    onSubmit() {
      ...
      if (dispositivoLocalizacion.length === 0) {
        ...
        if (!FunctionsUtils.pointInLand(p, this.coordenadaFinca)) {
          ...
          this.timeoutSnackbar = 4000;
          this.textSnackbar =
            'Localizacion marcada fuera de la finca. Por favor seleccione una localizacion dentro de la finca';
          this.snackbar = true;
          this.isLoading = false;
        } else {
          var t = this;
          ...
          if (dispositivoTipo === 'GPS') {
            ...
            if (this.map2) {
              if (
                Math.floor(this.coordenadaPivot.getLatLngs()[0].lat * Math.pow(10, 4)) / Math.pow(10, 4) ===
                  Math.floor(this.coordenadasDispositivo[1] * Math.pow(10, 4)) / Math.pow(10, 4) &&
                Math.floor(this.coordenadaPivot.getLatLngs()[0].lng * Math.pow(10, 4)) / Math.pow(10, 4) ===
                  Math.floor(this.coordenadasDispositivo[0] * Math.pow(10, 4)) / Math.pow(10, 4)
              ) {
                latlngCenter = L.latLng(
                  this.coordenadaPivot.getLatLngs()[1].lat,
                  this.coordenadaPivot.getLatLngs()[1].lng,
                );
                latlngPoint = L.latLng(
                  this.coordenadaPivot.getLatLngs()[0].lat,
                  this.coordenadaPivot.getLatLngs()[0].lng,
                );
              } else {
                latlngCenter = L.latLng(
                  this.coordenadaPivot.getLatLngs()[0].lat,
          ...
          if (dispositivoTipo === 'GPS') {
            dispositivo = new DispositivoGps(
              this.getDispositivo.key,
              dispositivoNombre,
              dispositivoId,
              dispositivoTipo,
    ...
    async eliminarDispositivo() {
      ...
      FactoryAPI.getFactoryAPI('Firebase')
        .getDispositivo()
        .deleteDevice(this.getDispositivo)
        .then((result) => {
    ...
    checkMeasurementsGps(idDevice: string) {
      ...
      FactoryAPI.getFactoryAPI('Firebase')
        .getMedida()
        .checkDeviceMeasurement(idDevice)
        .then((result) => {
            ...
            FactoryAPI.getFactoryAPI('Firebase')
              ...
              .then((result) => {
                ...
                FactoryAPI.getFactoryAPI('Firebase')
                  ...
                  .then((resultDispositivo:Dispositivo) => {
                    if (resultDispositivo !== null) {
                      ...
                      FactoryAPI.getFactoryAPI('Firebase')
                        .getPivot()
                        .pivotInformation(pivotDispositivo)
                        .then((result) => {
                          if (result !== null) {
                            ...
                            if (
                              ...
                              latlngCenter = L.latLng(
                                coordenadaPivot.getLatLngs()[1].lat,
                                coordenadaPivot.getLatLngs()[1].lng,
                              );
                              latlngPoint = L.latLng(
                                coordenadaPivot.getLatLngs()[0].lat,
                                coordenadaPivot.getLatLngs()[0].lng,
                              );
                            } else {
                              latlngCenter = L.latLng(
                                coordenadaPivot.getLatLngs()[0].lat,
  ...
  mounted() {
    ...
    if (this.getFinca === null || this.getPivot === null || this.getDispositivo === null) router.push('/finca');
      ...
      FactoryAPI.getFactoryAPI('Firebase')
        .getMedida()
        .listMeasurements(this.id, this.numberItems)
        .then((result) => {
          if (result !== null) {
            result.forEach(function (childResult:Medida) {
              var medida = childResult.medida;
