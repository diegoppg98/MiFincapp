<template>
  <div>
    ...
    <v-row>
      <v-col :cols="posMap">
        <v-form dark ref="form" v-model="valid" :lazy-validation="lazy">
          <v-text-field v-model="nombre" :rules="nameRules" label="Nombre" required clearable></v-text-field>
          <v-text-field
            v-model="id"
            :rules="nameRules"
            :error="isIdUsed"
          ...
          <v-switch
            v-show="showSwitch"
            v-model="validUbicacion"
            class="ma-4"
            label="Elige ubicación del dispositivo"
            color="grey"
          ></v-switch>
          <v-row justify="end">
            <v-btn
        ...
        if (this.getPivot !== null) {
          ...
          L.geoJSON(L.geoJSON(JSON.parse(this.localizacion[0])).toGeoJSON(), {
            onEachFeature: function (feature, layer) {
              ...
              else var p = L.latLng(feature.geometry.coordinates[1][1], feature.geometry.coordinates[1][0]);
  ...
  methods: {
    ...
    tipoChanged() {
      if (this.tipo === 'GPS') {
        ...
        if (this.marker) this.map2.removeLayer(this.marker);
        ...
        if (!FunctionsUtils.pointInLand(p, this.coordenadaFinca)) {
          ...
          if (dispositivo !== null) {
            FactoryAPI.getFactoryAPI('Firebase')
              .getDispositivo()
              .createDevice(dispositivo)
              .then((result) => {
                if (dispositivoTipo === 'GPS') this.checkMeasurementsGps(dispositivoId);
                if (dispositivoTipo === 'Suelo') this.checkMeasurementsSuelo(dispositivoId);
                        ...
                        .then((result) => {
                          if (result !== null) {
                            ...
                            if (
                              ...
                              Math.floor(coordenadaPivot.getLatLngs()[1].lat * Math.pow(10, 4)) / Math.pow(10, 4) ===
                                Math.floor(coordenadasActualesDispositivo[1] * Math.pow(10, 4)) / Math.pow(10, 4) &&
                              Math.floor(coordenadaPivot.getLatLngs()[1].lng * Math.pow(10, 4)) / Math.pow(10, 4) ===
                                Math.floor(coordenadasActualesDispositivo[0] * Math.pow(10, 4)) / Math.pow(10, 4)
                            ) {
                              latlngCenter = L.latLng(
                                coordenadaPivot.getLatLngs()[0].lat,
              ...
              .then((result) => {
                ...
                FactoryAPI.getFactoryAPI('Firebase')
                  ...
                  .then((resultDispositivo) => {
                    ...
                    var dispositivo = new DispositivoSuelo(
                      deviceKey,
                      nombreDispositivo,
                      idDispositivo,
                      tipoDispositivo,
  ...
  computed: {
    ...
    idUsed() {
      ...
      if (id === '') {
        ...
        resultado = FactoryAPI.getFactoryAPI('Firebase')
          .getDispositivo()
          .deviceExist(id)
