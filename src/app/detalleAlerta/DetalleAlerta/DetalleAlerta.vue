...
export default {
  ...
  var alerta = new Alerta(this.getAlerta.key,this.nombre, this.tipo, alertaDatos, alertaOpcion, this.slider, this.tiempoUltimaNotificacion, this.silenciated, this.finca.key, this.pivot.key, this.dispositivo.key);
  ...
  sliderChanged(){
    var alerta = new Alerta(this.getAlerta.key,this.nombre, this.tipo, this.rangeSuelo, this.suelo, this.slider, this.tiempoUltimaNotificacion, this.silenciated, this.finca.key, this.pivot.key, this.dispositivo.key);
       FactoryAPI.getFactoryAPI("Firebase").getAlerta().updateAlert(alerta).then((result) =>{
          this.isLoading = false;
          }).catch((error) => {
   	     this.colorSnackbar = "error";
  ...
  if(this.silenciated){ this.silenciated = false; this.icon = 'mdi-volume-high';}
     ...
     onSubmit() {
       ...
       if(tierraLocalizacion.length === 0){
                  ...
                  L.geoJSON( L.geoJSON(JSON.parse(tierraLocalizacion[0])).toGeoJSON(), {
                    ...
                    var layerAlerta = layer;
                     L.geoJSON( L.geoJSON(JSON.parse(dispositivoLocation)).toGeoJSON(), {
                         ...
                         FactoryAPI.getFactoryAPI("Firebase").getAlerta().updateAlert(alertaNotificada).then((result) =>{
                           this.changeAlerta(alerta); 
                           }).catch((error) => {
                           });
                       }).catch((error) => {   	                 
   	 ...
   	 this.colorSnackbar = "success";
         ...
         var NombreDispositivo = this.dispositivo.key;
         var alertaNombre = this.nombre;
         var alertaDatos = this.rangeSuelo; //////////////////7
    ...
    async eliminarAlerta() {
      this.dialog = false;
      var alertaNombre = this.nombre;
      FactoryAPI.getFactoryAPI("Firebase").getAlerta().deleteAlert(this.getAlerta.key).then((result) =>{
          router.push('/alerta');
          this.isLoading = false;
          }).catch((error) => {
   	     this.colorSnackbar = "error";
...
//   var lc = document.getElementsByClassName('leaflet-control-layers');
 //  lc[0].style.visibility = 'hidden';
  ...
  var alerta = new Alerta(this.getAlerta.key,this.nombre, this.tipo, tierraLocalizacion, this.gps, this.slider, this.tiempoUltimaNotificacion, this.silenciated, this.finca.key, this.pivot.key, this.dispositivo.key);
       FactoryAPI.getFactoryAPI("Firebase").getAlerta().updateAlert(alerta).then((result) =>{          
          }).catch((error) => {
   	     this.colorSnackbar = "error";
             this.textSnackbar = 'Error al actualizar la alerta. Por favor inténtelo otra vez';
  ...
  this.dispositivoGps = false;
    ...
    // this.changeUser(true);  
     ...
     if(this.getAlerta === null) router.push('/alerta');
        ...
        var pivots = this.listaPivots;
        FactoryAPI.getFactoryAPI("Firebase").getPivot().listPivots(this.fincaKey).then((result) =>{
         if(result !== null){
           result.forEach(function(childResult) {       
              var pivot = childResult.nombre;         
              pivots.push({ text: pivot , value: childResult });
    ...
    var t = this;
        var dispositivos = this.listaDispositivos;
        FactoryAPI.getFactoryAPI("Firebase").getDispositivo().listDevices(this.pivotKey).then((result) =>{
          if(result !== null){
           result.forEach(function(childResult) {       
              var dispositivo = childResult.nombre;
              dispositivos.push({ text: dispositivo , value: childResult }); 
